...
export const Route = createFileRoute("/auth")({
  ...
  head: () => ({
    meta: [
      { title: "Acessar O Plano — Planner de Estudos" },
      { name: "description", content: "Ative ou acesse sua conta no O Plano, o planner de estudos para concursos." },
      { property: "og:title", content: "Acessar O Plano — Planner de Estudos" },
      { property: "og:description", content: "Ative ou acesse sua conta no O Plano, o planner de estudos para concursos." },
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-secondary px-4 py-8">
      <Link to="/" className="flex items-center gap-2.5 justify-center mb-8">
        ...
        <span className="font-display font-bold text-2xl text-secondary-foreground">O Plano</span>
      </Link>

      <Card className="w-full max-w-md border-primary/25 p-5 sm:p-7">
        <Tabs value={activeTab} onValueChange={(v) => {
          setActiveTab(v as "primeiro-acesso" | "aluno");
          resetFields();
        }} className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-5 h-11 bg-muted rounded-md p-1">
            <TabsTrigger value="primeiro-acesso" className="rounded text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              Primeiro Acesso
            </TabsTrigger>
            <TabsTrigger value="aluno" className="rounded text-sm font-semibold data-[state=active]:bg-primary data-[state=active]:text-primary-foreground">
              Já sou Aluno
            </TabsTrigger>
          </TabsList>
